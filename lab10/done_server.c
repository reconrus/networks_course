...
pthread_mutex_t mutex_cdb;
pthread_mutex_t mutex_bdb;
...
void server(){
    ...
    while (1) {
        ...
        if (FD_ISSET(master_sock_tcp_fd, &readfds)) {
		    ...
		    if(thread >= 0){
                node_info_t* node_info= malloc(sizeof(node_info_t));
...
int main(){
    ...
    map_init(&map);
    map_init(&cdb);
    bdb = malloc(sizeof(list_t));
    
    pthread_mutex_init(&mutex_cdb, NULL);
    pthread_mutex_init(&mutex_bdb, NULL);
